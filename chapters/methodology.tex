...
practice there are two separate leaflet meshes, referred to here as \ph{Mesh5}
and \ph{Mesh6}.
...
background voxel, in other words the distance to the closest wall. In the middle
...
that. Ultrasound volumes have speckle and blurred edges, and this scan has an
...
The mesh and the volume did not overlap when they were first loaded together;
they sat in different coordinate systems. This alignment matters here because, as
...
from the same volume. I added planes to the mesh that correspond to this bounding
box, so that the mesh carries the same box as a built-in reference. Matching the
...
only at the spot being measured. Tissue was defined as an intensity of about~0.2
...
first rises to~0.2 and ends where it falls back below~0.2, and the thickness is
...
\begin{table}[htbp]
  ...
  \begin{tabular}{clcc}
    ...
    \midrule
    1 & Base               & \ph{Mesh5} & \ph{} \\
    2 & Middle             & \ph{Mesh5} & \ph{} \\
    3 & Free edge (median) & \ph{Mesh5} & \ph{} \\
    4 & Centre (median)    & \ph{Mesh5} & \ph{} \\
    \dots & \dots          & \dots      & \dots \\
    \bottomrule
  \end{tabular}
...
The interpolation was carried out in Blender with a short Python script. numpy is
...
only hold one thickness value and one vertex group. So \ph{Mesh5} and \ph{Mesh6}
