...
interpolation, were done with short Python scripts using the numpy and scipy
libraries. The following sections describe each part, and, more importantly, why
...
With the mesh aligned, the thickness was measured in 3D~Slicer. The tool used is
...
For the interpolation I chose inverse distance weighting (IDW). The idea is easy
...
to explain and defend in full. For the power parameter I used a value of one,
which gives smooth, gradual transitions. A higher power tends to make the values
cluster tightly around each known point, which produces unnatural-looking
``bubbles'' when there are only a few points to work from.

The interpolation was carried out in Blender with a short Python script as shown in Listing~\ref{lst:thickness_interpolation}. numpy is
used for the distance and averaging calculations, and scipy is used for handling
the nearest-point lookups. For every vertex the script computes the distances to
...
For this step, Blender's Solidify modifier was used. This modifier takes a
...
Jetzt wo erfolgreich eine gute approximation beziehungsweise Modell der Mitral Klappe erzielt worden war ging es zum eigentlichen Ziel und Kern dieser Arbeit: Ein realitätsnahes Ultrashall Phantom zu fabrizieren. Nachdem Präzision und realitätstreue im Vordergrund über leichte und massentaugliche Produktion standen, wurde FDM 3D-Druck gewählt. 3D-Drucker ermöglichen mitleiweile eine ausreichende Detailtreue und zudem auch die für dieses Projekt essentielle Möglichkeit mit flexiblen Materialien wie TPU zu arbeiten. Die herstellung von flexiblen Strukturen wäre auch mit Gußformen und einem Material wie beispielsweise Silikon möglich und würden sogar eine massentaugliche Herstöllung ermöglichen jedoch blieb die Wahl beim 3D-Druck da dadurch leichte anpassungen möglich wären und zudem nicht darauf rücksicht genommen werden muss, wie man die Gussform entfernt.

Schon während der Abmessung der Dickenwerte über die Laufzeit des Projekts wurde zeitgleich mit Testdrucken parallel ermittelt, wie man zu einem bestmöglichen Ergebnis kommen kann. Es war klar, dass die Herzklappe beim Druck Support Strukturen benötigen wird, da die convexe Form möglichst flach auf den Druckbrett aufliegen müsste und es damit Überhänge weit über 60° gäben würde. Das Modell senkrecht zu drucken verminderte zwar die querschnittsfläche an Support ein wenig aber nicht maßgiblich und zudem wurde die Struktur dann beim drucken instabil da sich das Druckbrett in der Y achse hin und her bewegt. Das Problem mit Support Material ist, dass es beim entfernen schwierig ist, alle Rückstende zu beseitigen und eine glatte Fläche zu erziehlen. In diesem Fall beim drucken 